...
module todolist::todolist_tests{
    ...
    fun test_todolist(){
        ...
        {   
            ...
            let mut userTable = test_scenario::take_shared<Table<address,vector<TodoObject>>>(scenario);
            let ctx = test_scenario::ctx(scenario);
            ...
            let  (mut taskID, mut taskName, mut checked, mut deleted) = todolist::get_TodoObject(task);
            assert!(taskID == 0, 100);
